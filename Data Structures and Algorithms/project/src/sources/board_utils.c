...
int check_square_data(int position, char type, int target, int board_size) {
    ...
    if(position > board_size-2 || position < 1){
        ...
        status = INVALID_SQUARE_DATA;
    } else if (type == 'L' && position > target){
        status = INVALID_SQUARE_DATA;
...
int read_square_line(Board* board, FILE* fd) {
    ...
    char type;
    int target;
    int matched = sscanf(buffer, "%d%c%d", &position, &type, &target);
    if(matched == 3){
        status = check_square_data(position, type, target, get_size(board));
        if(status == SUCCESS){
            //Get the square and set the new target
...
int read_board_file(Board* board, FILE* fd) {
    ...
    int columns;
    char x;
    int matched = sscanf(buffer, "%d%c%d", &rows,&x, &columns);
    int count = 2;
    if(matched == 3){
...
int load_board(Board* board, char* path) {
    ...
    if(fd == NULL){
        ...
        status = read_board_file(board, fd);
        fclose(fd);
    }
    return status;
}
...
int get_rows(Board* board) {
    return board->rows;//Dame la info en cambio set es establece este valor
}
...
void set_rows(Board* board, int rows) {
    board->rows= rows;//CUANDO PUNTO Y CUANDO -> : siempre con punto excepto cuando es a traves de puntero
}
